			...
			const optics::Aperture& aperture)
		{
			std::visit(
				[](const auto& shape)
				{
					...
					{
						if (
							!positiveFinite(shape.radiusX) ||
							!positiveFinite(shape.radiusY))
						{
							throw std::invalid_argument(
								"Elliptical launch pupil radii must be positive.");
						}
					}
					else
					{
			...
			double v)
		{
			return std::visit(
				[u, v](const auto& shape) -> glm::dvec2
				{
					...
					{
						//
						// Uniform-area sampling of an ellipse.
						//
						// Start with uniform-area sampling of the unit disk and
						// independently scale X/Y by the ellipse semiaxes.
						//
						const double radius =
							std::sqrt(u);
